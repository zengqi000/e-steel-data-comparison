...
2. **销售提单业务检查汇总表** (`/sell-pickup/scroll-check`)
   - 对比：`sumWeight` 之和 vs `newPickupWeight`
   - 对比：`sumAmount` 之和 vs `newPickupSumAmount`（新增金额对比）
...
脚本输出**详细**文字格式的核对结果，每个合同包含以下内容：

**合同基本信息：**
- SAP编码、品牌、材质、规格、合同数量/单价/金额等（从合同检查接口获取）

**数据核对表：**
- 提单重量：合同检查 vs 销售提单业务表
- 提单金额：合同检查 vs 销售提单业务表（含交叉比对）
- 实提重量：合同检查 vs 销售实提业务表
- 销售退货重量：合同检查 vs 销售退货报表

**子表记录明细（每张子表独立展示）：**
- 记录条数统计
- 合计重量和合计金额
- 每条记录的重量、金额、材质、规格、仓库/品牌等详细信息
- 超过20条时显示前20条，标注剩余数量

✅ 表示数据一致，❌ 表示有差异
末尾显示汇总信息：合同总数、差异条数及具体差异明细
...
- 调用4个 API 接口获取数据：
  ...
  4. 销售退货报表（`/sell-return-product/page`）
- 对比4个关键字段：
  - 提单重量（newPickupWeight vs 销售提单.sumWeight）
  - 提单金额（newPickupSumAmount vs 销售提单.sumAmount）**新增**
  - 实提重量（newRealPickWeight vs 销售实提.sumWeight）
  - 销售退货重量（newSellReturnWeight vs 销售退货.weight）
- 展示合同基本信息（SAP编码、品牌、材质、规格等）
- 展示每个子表的记录明细（条数、每条重量/金额/材质/规格等）
- 识别重量差异（容忍度：0.001 吨）和金额差异（容忍度：0.01 元）
